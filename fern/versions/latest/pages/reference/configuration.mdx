...
### Reusing component virtual environments

`skip_venv_if_present=true` reuses a component's virtual environment only after Gym has successfully completed dependency installation. Servers sharing the same environment wait for that setup to finish before starting; different environments can install concurrently.

Gym records successful setup in `.venv/.nemo-gym-setup-complete`. An existing environment without that marker runs dependency installation once, without Gym deleting the environment. Failed or interrupted setup leaves no completion marker, so the next invocation retries installation. Do not create the marker manually: the Python executable and activation script can exist before dependencies finish installing.

For prebuilt environments, run `gym env prefetch` with the intended configuration using the updated Gym version before packaging them. Setup requires a writable environment and parent directory, and reuse must be able to open the sibling `.venv.setup.lock` file. The filesystem must support process-shared file locking. Keep the lock file in place while any Gym setup may be running.

`skip_venv_if_present=false` still requests setup on every invocation. It serializes installers but does not protect an already-running server from a separately requested reinstall; stop servers using that environment before forcing setup.
