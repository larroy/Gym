    ...
    request = AgentSeedSessionRequest(
        ...
        tool_accesses=[
            ...
            MCPToolAccess(
                name="task-tools",
                required=True,
                connection=MCPStreamableHTTPTransport(
...
def test_agent_tool_accesses_require_absolute_urls_and_unique_names() -> None:
    duplicate = [
        MCPToolAccess(
            name="tools",
            required=True,
            connection=MCPStreamableHTTPTransport(url="http://resources:8000/mcp"),
        ),
        MCPToolAccess(
            name="tools",
            required=False,
            connection=MCPStdioTransport(command="optional-tools"),
        ),
    ]
    ...
    with pytest.raises(ValidationError, match="valid URL"):
        MCPStreamableHTTPTransport(url="/mcp")
...
def test_episode_tool_access_overrides_configured_access_by_name() -> None:
    ...
    episode = MCPToolAccess(
        name="memory",
        required=True,
        connection=MCPStreamableHTTPTransport(url="http://episode:8000/mcp"),
    )
    request = AgentSeedSessionRequest(
        episode_id=EpisodeId(rollout_id="rollout"),
        task_id=TaskId(taskset="test", task_id="task"),
...
def test_empty_seed_response_preserves_legacy_wire_shape() -> None:
    ...
    with pytest.raises(ValidationError):
        MCPStdioTransport(command="server", unknown=True)
