...
) -> tuple[SingleAgentEpisodeProcessor, _Client]:
...
async def test_single_agent_protocol_with_direct_resources_tools() -> None:
    processor, client = _processor(resources_tool_transports=["direct_http"])
...
async def test_single_agent_translates_resources_mcp_metadata_to_canonical_tool_access() -> None:
    processor, client = _processor(resources_tool_transports=["direct_http", "mcp"])
